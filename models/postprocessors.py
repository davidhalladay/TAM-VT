...
from models.segmentation_detr import PostProcessSegm
...
class PostProcessSTVG(nn.Module):
    ...
    def forward(self, outputs, frames_id=None, video_ids=None, time_mask=None):
        ...
        return pred_steds, score.exp().item()

    @torch.no_grad()
    def forward_w_pred_sted_arg(self, pred_sted, frames_id=None, video_ids=None, time_mask=None):
        return self.forward({"pred_sted": pred_sted}, frames_id, video_ids, time_mask)
...
def build_postprocessors(args, dataset_name) -> Dict[str, nn.Module]:
    ...
    if dataset_name in [
        "vidstg", "hcstvg", "vq2d", "nlq", "mq", "mq_slowfast_features",
        "type2", "vidstg_adapted", "hcstvg_adapted",
    ]:
        postprocessors[dataset_name] = PostProcessSTVG()
    elif dataset_name in ["mq_w_slowfast_features"]:
        postprocessors["mq"] = PostProcessSTVG()

    if dataset_name in ["vos"]:
