...
from .mask2former_layers_last_layer_too_ms import build_pixel_decoder
...
class Transformer(nn.Module):
    def __init__(
        ...
        # )
        self.encoder = VTMMatchingModule(dim_w=256, dim_z=256, n_attn_heads=4, n_levels=2, args=args)
        ...
        if self.args.model.vost.parts_exp.decoder.multiple_queries.enable:
            # import ipdb; ipdb.set_trace()
            self.parts_embed = nn.Embedding(
                self.args.model.vost.parts_exp.decoder.multiple_queries.num_queries_per_frame,
                d_model
            )
    ...
    def _reset_parameters(self):
        for n, p in self.named_parameters():
            if 'rel_time_theta' in n:
                continue
            if 'abs_time_theta' in n:
                continue
            if p.dim() > 1:
    ...
    def forward(
        ...
        **kwargs
        ...
        if encode_and_save:
            ...
            # add parts embedding
            if self.args.model.vost.parts_exp.decoder.multiple_queries.enable:
                # repeat s.t. every frame has parts queries
                # import ipdb; ipdb.set_trace()
                parts_embed = self.parts_embed.weight.repeat(query_embed.shape[0], 1)
                parts_embed = parts_embed.unsqueeze(1)

                query_embed = query_embed.repeat_interleave(
                    self.args.model.vost.parts_exp.decoder.multiple_queries.num_queries_per_frame,
                    dim=0)

                query_embed = query_embed + parts_embed
            ...
            if task_name == "vost":
                ...
                W_Ss_all_levels = []
                Z_Ss_all_levels = []
                # TODO: check if this should be used for final version
                Z_Ss_bg_all_levels = []
                for __level in range(2):
                    ...
                    for _ind_time, _mem in kwargs["memory_encoded"].items():
                        ...
                        Z_Ss_bg.append(((_mem['mask_bg']['src_last_two_layers'][__level]) + _mem['mask_bg']['pos_last_two_layers'][__level]))  # [1, 256, 15, 25]

                    W_Ss = torch.cat(W_Ss)  # [N, 256, 15, 25]
                    Z_Ss = torch.cat(Z_Ss)  # [N, 256, 15, 25]
                    Z_Ss_bg = torch.cat(Z_Ss_bg)  # [N, 256, 15, 25]

                    W_Ss = W_Ss.unsqueeze(0).unsqueeze(0)
                    Z_Ss = Z_Ss.unsqueeze(0).unsqueeze(0)
                    Z_Ss_bg = Z_Ss_bg.unsqueeze(0).unsqueeze(0)

                    W_Ss_all_levels.append(W_Ss)
                    Z_Ss_all_levels.append(Z_Ss)
                    Z_Ss_bg_all_levels.append(Z_Ss_bg)

                abs_time_encoding = None
                if self.args.model.vost.memory.abs_time_encoding:
                    abs_time_encoding = [*kwargs["memory_encoded"].keys()]
                    abs_time_encoding = [max(abs_time_encoding) + 1 - e for e in abs_time_encoding]
                ...
                for _t in range(W_Qs[0].shape[1]):
                    ...
                    Z_Qs.append(self.encoder([W_Qs[0][:, _t][None], W_Qs[1][:, _t][None]], W_Ss_all_levels, Z_Ss_all_levels, 
                                                    abs_time_encoding=abs_time_encoding, Z_Ss_bg=Z_Ss_bg_all_levels))

                # Z_Qs has the same shape as W_Qs
            ...
            dict_features = {
                k: v.tensors
                for k, v in zip(self.args.model.vost.pixel_decoder.in_features, kwargs['features'])
            }
            mask_features, _, multi_scale_features \
                = self.pixel_decoder.forward_features(dict_features, img_only_memory_reshaped)
            ...
            # multiple parts queries per frame
            if self.args.model.vost.parts_exp.decoder.multiple_queries.enable:
                factor_repeat = self.args.model.vost.parts_exp.decoder.multiple_queries.num_queries_per_frame
                ...
                # repeat interleave variables
                assert img_memory is None
                assert query_mask is None
                assert text_memory is None
                assert len(text_mask) == 0
                assert memory_mask is None
                mask_rep = mask.repeat_interleave(factor_repeat, dim=0)
                pos_embed_rep = pos_embed.repeat_interleave(factor_repeat, dim=0)

                pixel_decoder_out_rep = {}
                pixel_decoder_out_rep['mask_features'] = \
                    kwargs["pixel_decoder_out"]['mask_features'].repeat_interleave(factor_repeat, dim=0)
                pixel_decoder_out_rep['multi_scale_features'] = [
                    elem.repeat_interleave(factor_repeat, dim=0)
                    for elem in kwargs["pixel_decoder_out"]['multi_scale_features']
                ]
                pixel_decoder_out_rep['pos_features'] = [
                    elem.repeat_interleave(factor_repeat, dim=0)
                    for elem in kwargs["pixel_decoder_out"]['pos_features']
                ]
                ...
                hs = self.decoder(
                    ...
                    memory_key_padding_mask=mask_rep,  # (b*t)xn_tokens
                    pos=pos_embed_rep,  # n_tokensx(b*t)xF
                    query_pos=query_embed,  # n_queriesx(b*t)xF
                    tgt_key_padding_mask=query_mask,  # bx(t*n_queries)
                    text_memory=text_memory,
                    text_memory_mask=text_mask,
                    memory_mask=memory_mask,
                    pixel_decoder_out=pixel_decoder_out_rep,
                )  # n_layersxn_queriesx(b*t)xF

                # merge hs parts (condition on self.training)
                hs_merge = {}
                hs_merge['pred_masks'] = hs['pred_masks'].reshape(
                    (-1, factor_repeat) + hs['pred_masks'].shape[1:])
                hs_merge['pred_masks'] = hs_merge['pred_masks'].mean(1)

                hs_merge['aux_outputs'] = [
                    {'pred_masks': elem['pred_masks'].reshape(
                        (-1, factor_repeat) + elem['pred_masks'].shape[1:]).mean(1)}
                    for elem in hs['aux_outputs']
                ]

                assert not self.return_weights
