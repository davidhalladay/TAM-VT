...
from .mask2former_layers import build_pixel_decoder
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.encoder = VTMMatchingModule(dim_w=256, dim_z=256, n_attn_heads=4, n_levels=1)
        ...
        decoder_layer = TransformerDecoderLayer(
            ...
            activation,
            no_tsa=no_tsa,
            attention_type=self.args.model.static.attention_decoder,
        )
        ...
        **kwargs
        ...
        if encode_and_save:
            ...
            if task_name == "static":
                ...
                W_Qs = (src + pos_embed).unsqueeze(0).unsqueeze(2)

                W_Ss = []
                Z_Ss = []
                for _ind_time, _mem in kwargs["memory_encoded"].items():
                    W_Ss.append((_mem['image']['src_last_layer'] + _mem['image']['pos_last_layer']))  # [1, 256, 15, 25]
                    Z_Ss.append((_mem['mask']['src_last_layer'] + _mem['mask']['pos_last_layer']))  # [1, 256, 15, 25]
                ...
                Z_Qs = []
                for _t in range(W_Qs.shape[1]):
                    Z_Qs.append(self.encoder([W_Qs[:, _t][None]], [W_Ss], [Z_Ss])[0])
            ...
            # PIXEL DECODER
            assert len(self.args.model.static.pixel_decoder.in_features) == len(kwargs['features'])
            dict_features = {'res5': img_only_memory_reshaped}
            dict_features.update({
                k: v.tensors
                for k, v in zip(self.args.model.static.pixel_decoder.in_features[:-1], kwargs['features'][:-1])
            })
