...
class TubeDETR(nn.Module):
    ...
    def __init__(
        ...
        if self.args.model.vost.use_second_last_feature:
            backbone.num_channels = backbone.num_channels // 2

        # self.input_proj = nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1)

        self.input_proj = nn.ModuleDict({
            "res5": nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1),
            "res4": nn.Conv2d(backbone.num_channels // 2, hidden_dim, kernel_size=1),
        })
        ...
        if "vost" in self.args.tasks.names:
            assert self.args.model.vost.use_projection_layer
            if self.args.model.vost.use_projection_layer:
                # self.input_proj_query_image = nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1)
                # self.input_proj_query_mask = nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1)

                # self.input_proj_query_image = nn.ModuleDict({
                #     "res5": nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1),
                #     "res4": nn.Conv2d(backbone.num_channels // 2, hidden_dim, kernel_size=1),
                # })

                if self.args.model.vost.get("separate_input_proj_for_all"):
                    self.input_proj_mem_image = nn.ModuleDict({
                        "res5": nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1),
                        "res4": nn.Conv2d(backbone.num_channels // 2, hidden_dim, kernel_size=1),
                    })
                    self.input_proj_mem_mask = nn.ModuleDict({
                        "res5": nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1),
                        "res4": nn.Conv2d(backbone.num_channels // 2, hidden_dim, kernel_size=1),
                    })
                else:
                    self.input_proj_query_mask = nn.ModuleDict({
                        "res5": nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1),
                        "res4": nn.Conv2d(backbone.num_channels // 2, hidden_dim, kernel_size=1),
                    })
    ...
    def forward(
        ...
        **kwargs,
        ...
        if encode_and_save:
            ...
            # features is a NestedTensor with [N, C, h, w] and [N, h, w] for tensors and mask respectively
            # pos is a position embedding list of [N, C, h, w] for each feature level
            ...
            # if self.args.debug: import ipdb; ipdb.set_trace()
            # encode reference crop and memory
            if task_name == "vost":
                for _ind_time, _mem in kwargs["memory"].items():
                    ...
                    # src_mem_image = self.input_proj_query_image(src_mem_image)  # torch.Size([8, 256, 5, 6])
                    if self.args.model.vost.get("separate_input_proj_for_all"):
                        src_mem_image = [
                            self.input_proj_mem_image["res5"](features_mem_image[-1].tensors),
                            self.input_proj_mem_image["res4"](features_mem_image[-2].tensors)
                        ]
                    else:
                        src_mem_image = [
                            self.input_proj["res5"](features_mem_image[-1].tensors),
                            self.input_proj["res4"](features_mem_image[-2].tensors)
                        ]
                    ...
                    # print(_mem_mask_inflated.tensors.shape)
                    # if (_mem_mask_inflated.tensors > 0.5).sum() > 0:
                    #     print((_mem_mask_inflated.tensors[_mem_mask_inflated.tensors > 0.5]).mean())
                    
                    
                    features_mem_mask, pos_mem_mask = self.backbone(_mem_mask_inflated)
                    # src_mem_mask, mask_mem_mask = features_mem_mask[-1].decompose()

                    # src_mem_mask = self.input_proj_query_mask(src_mem_mask)  # torch.Size([8, 256, 5, 6])
                    if self.args.model.vost.get("separate_input_proj_for_all"):
                        src_mem_mask = [
                            self.input_proj_mem_mask["res5"](features_mem_mask[-1].tensors),
                            self.input_proj_mem_mask["res4"](features_mem_mask[-2].tensors)
                        ]
                    else:
                        src_mem_mask = [
                            self.input_proj_query_mask["res5"](features_mem_mask[-1].tensors),
                            self.input_proj_query_mask["res4"](features_mem_mask[-2].tensors)
                        ]

                    # mem bg mask
                    _bg_mem_mask_inflated = NestedTensor((1.-_mem["mask"].tensors.repeat(1, 3, 1, 1)), _mem["mask"].mask)

                    features_bg_mem_mask, pos_bg_mem_mask = self.backbone(_bg_mem_mask_inflated)
                    # src_mem_mask, mask_mem_mask = features_mem_mask[-1].decompose()

                    # src_mem_mask = self.input_proj_query_mask(src_mem_mask)  # torch.Size([8, 256, 5, 6])
                    if self.args.model.vost.get("separate_input_proj_for_all"):
                        src_bg_mem_mask = [
                            self.input_proj_mem_mask["res5"](features_bg_mem_mask[-1].tensors),
                            self.input_proj_mem_mask["res4"](features_bg_mem_mask[-2].tensors)
                        ]
                    else:
                        src_bg_mem_mask = [
                            self.input_proj_query_mask["res5"](features_bg_mem_mask[-1].tensors),
                            self.input_proj_query_mask["res4"](features_bg_mem_mask[-2].tensors)
                        ]

                    # updating memory
                    _mem_encoded = {"image": {}, "mask": {}, "mask_bg": {}}
                    _mem_encoded["image"]["src_last_two_layers"] = src_mem_image
                    _mem_encoded["image"]["pos_last_two_layers"] = [pos_mem_image[-1], pos_mem_image[-2]]

                    _mem_encoded["mask"]["src_last_two_layers"] = src_mem_mask
                    _mem_encoded["mask"]["pos_last_two_layers"] = [pos_mem_mask[-1], pos_mem_mask[-2]]

                    _mem_encoded["mask_bg"]["src_last_two_layers"] = src_bg_mem_mask
                    _mem_encoded["mask_bg"]["pos_last_two_layers"] = [pos_bg_mem_mask[-1], pos_bg_mem_mask[-2]]
            ...
            src = self.input_proj["res5"](src)  # torch.Size([8, 256, 5, 6])
            kwargs["curr_image_encoded"] = {
                "src_last_two_layers": [src, self.input_proj["res4"](features[-2].tensors)],
                "pos_last_two_layers": [pos[-1], pos[-2]]
            }
...
class SetCriterion(nn.Module):
    ...
    def loss_masks_detr(self, outputs, targets, num_boxes, device):
        ...
        if self.args.loss_coef.vost.reweighting:
            tau = self.args.loss_coef.vost.reweighting_tau
            if self.args.loss_coef.vost.reweighting_type == 'area':
                # calculate area change for frame_weights
                delta_mask_gt = (target_masks.sum(1)[1:]-target_masks.sum(1)[:-1]).abs() + 1. # plus 1 to prevent nan
            elif self.args.loss_coef.vost.reweighting_type == 'cc':
                # calculate connected compoenets change for frame_weights
                cc_list = []
                for ii in range(len(target_masks)):
                    img = target_masks[ii].cpu().numpy().astype(np.uint8)
                    num_labels, _ = cv2.connectedComponents(img, connectivity=4)
                    cc_list.append(num_labels)
                cc_list = torch.tensor(cc_list).to(device) 
                delta_mask_gt = (cc_list[1:] - cc_list[:-1]).abs() + 1. # plus 1 to prevent nan
            elif self.args.loss_coef.vost.reweighting_type == 'cm':
                # calculate center of mass change for frame_weights
                # Find indices where we have mass
                cent_x_list = []
                cent_y_list = []
                for ii in range(len(target_masks)):
                    mass_x, mass_y = torch.where(target_masks[ii].reshape(h, w) > 0.)
                    mass_x, mass_y = mass_x.float(), mass_y.float()
                    if len(mass_x) > 0:
                        cent_x = mass_x.mean() - mass_x.min()
                        cent_y = mass_y.mean() - mass_y.min()
                        cent_x_list.append(cent_x)
                        cent_y_list.append(cent_y)
                    else:
                        if len(cent_x_list) > 0:
                            cent_x_list.append(cent_x_list[-1])
                            cent_y_list.append(cent_y_list[-1])
                        else:
                            print("weird case found: the first frame has no any foregorund obj")
                            cent_x_list.append(0)
                            cent_y_list.append(0)
                cent_x_list = torch.tensor(cent_x_list).to(device) 
                cent_y_list = torch.tensor(cent_y_list).to(device) 
                d_cent_x = (cent_x_list[1:] - cent_x_list[:-1]).abs() + 1.
                d_cent_y = (cent_y_list[1:] - cent_y_list[:-1]).abs() + 1.
                cm_dist = (d_cent_x**2 + d_cent_y**2)**0.5
                delta_mask_gt = cm_dist
            else:
                raise ValueError(f"Unknown reweighting type: {self.args.loss_coef.vost.reweighting_type}")
            delta_demean = delta_mask_gt/delta_mask_gt.mean()
            frame_weights = torch.exp(delta_demean.abs()/tau)/torch.exp(delta_demean.abs()/tau).mean()
            frame_weights = torch.cat((torch.ones((1,)).to(device), frame_weights), dim=0)
        else:
            frame_weights = None

        # TODO: this is only for testing, visualization. and should be removed.
        # print(frame_weights)
        # target_masks_plt = target_masks.reshape(T, _, h, w)[1:-1, :, :, :]
        # target_masks_plt_grid = torchvision.utils.make_grid(target_masks_plt, nrow=8, value_range=(0, 1))
        # torchvision.utils.save_image(target_masks_plt_grid, 'test.png')
        # exit()
        
        losses = {
            "loss_mask": sigmoid_focal_loss(
                src_masks, target_masks, num_boxes,
                alpha=self.args.loss_coef.vost.mask_alpha, 
                frame_weights=frame_weights if self.args.loss_coef.vost.reweighting_focal else None
            ),
            "loss_dice": dice_loss(
                src_masks, target_masks, num_boxes, 
                frame_weights=frame_weights if self.args.loss_coef.vost.reweighting_dice else None
            ),
        }
...
def build_tubedetr(args, backbone, transformer):
    ...
    losses = []
    losses.append("masks")
    weight_dict["loss_mask"] = args.loss_coef.vost.mask
    weight_dict["loss_dice"] = args.loss_coef.vost.dice
