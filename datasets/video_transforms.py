...
class ComposeMultiScale(object):
    def __init__(self, transforms):
        self.transforms = transforms

    def __call__(self, video, targets, scale):
        for t in self.transforms:
            video, targets = t(video, targets, scale)
        return video, targets
...
def resize_w_scale(clip, targets, size, max_size=None, scale=1.0):
    # size can be min_size (scalar) or (w, h) tuple
...
def resize_restrict_size(clip, targets, scale, max_size=None):
    # size can be min_size (scalar) or (w, h) tuple
...
class TransformWithScale():
    def __init__(self, transform):
        self.transform = transform

    def __call__(self, video, target=None, scale=1.0):
        return self.transform(video, target)


class MultiRestricSizePrev(object):
    def __init__(self, sizes, max_size=None):
        assert isinstance(sizes, (list, tuple))
        self.sizes = sizes
        self.max_size = max_size
    ...
    def __call__(self, video, target=None, scale=1.0):
        size = random.choice(self.sizes)
        return resize_w_scale(video, target, size, self.max_size, scale)


class MultiRestricSize(object):
    ...
    def __call__(self, video, target=None, scale=1.0):
        # video_agg, target_agg = [], []
        # for sc in self.scales:
        #     video_, target_ = resize_restrict_size(video, target, sc, self.max_size)
        #     video_agg.append(video_)
        #     target_agg.append(target_)
        # return video_agg, target_agg
        return resize_restrict_size(video, target, scale, self.max_size)
...
# class TransformApplyOnList:
#     def __init__(self, transform):
#         self.transform = transform

#     def __call__(self, video, target=None):
#         video_agg, targets_agg = [], []
#         for _v, _t in zip(video, target):
#             __v, __t = self.transform(_v, _t)
#             video_agg.append(__v)
#             targets_agg.append(__t)

#         return video_agg, targets_agg


def make_video_transforms(image_set, cautious, TTA=False, resolution=224):
    """
    ...
    """
    AOT multi scale transform
        {'align_corners': True,
        'flip': False,
        'max_size': 1040.0,
        'max_stride': 16,
        'min_size': None,
        'multi_scale': [1.0, 1.1, 1.2, 0.9, 0.8]}
    """
    ...
    if resolution == 128:
        ...
        scales = [480, 512, 544, 576, 608, 624]
        max_size = 1110
        resizes = [400, 500, 600]
        crop = 384
        test_size = [624]
    ...
    if image_set in ["val", "test"]:
        ...
        if TTA:
            return ComposeMultiScale(
                [
                    MultiRestricSizePrev(test_size, max_size=max_size),
                    TransformWithScale(normalizeop),
                ]
            )
        else:
            return Compose(
