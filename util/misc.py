...
def video_collate_fn_vos(do_round, div_vid, batch):
    ...
    final_batch["memory_images"] = NestedTensor.from_tensor_list(
        [tmp_target["memory_images"] for tmp_target in batch[2]], do_round
    )
    final_batch["memory_masks"] = NestedTensor.from_tensor_list(
        [tmp_target["memory_masks"] for tmp_target in batch[2]], do_round
    )
    final_batch["video_ids"] = [tmp_target["video_id"] for tmp_target in batch[2]]
    final_batch["frames_id"] = [tmp_target["frames_id"] for tmp_target in batch[2]]
    final_batch["scale"] = [tmp_target["scale"] for tmp_target in batch[2]]
...
def video_collate_fn_vost(do_round, div_vid, batch):
    ...
    """
    summary of batch:
    3 elements: samples, targets, memory
    samples: list of tensors, each tensor is a clip of frames of a video. size: BS x C x T x H x W
    targets: list of list of tensors, each tensor is a target for a clip. size: BS x T x H x W
        where T is the number of frames in the clip, which is different for each clip.
    memory: list of dicts, each dict contains memory images and masks for a video. size: BS x C x T x H x W
        the key of memory is "memory_images" and "memory_masks".
    """
    batch = list(zip(*batch)) # list of 3 elements: samples, targets, memory

    final_batch = {}
    # convert samples list of tensors to a tensor
    final_batch["samples"] = NestedTensor.from_tensor_list(batch[0], do_round)  # padded on the longest height and width

    # convert targets list of list of tensors to a list of tensors
    final_batch["durations"] = [len(x) for x in batch[1]]
...
def video_collate_fn_static(do_round, div_vid, batch):
    ...
    final_batch = {}
    final_batch["samples"] = NestedTensor.from_tensor_list(batch[0], do_round)  # padded on the longest height and width
    final_batch["durations"] = [len(x) for x in batch[1]]  # used to temporally unpad samples later
    final_batch["targets"] = [
        target for clip in batch[1] for target in clip
    ]  # flatten list of list of targets (one list per video), not temporally padded
    final_batch["memory_images"] = NestedTensor.from_tensor_list(
...
class NestedTensor(object):
    ...
    def _get_shape(self):
        return {"tensor_shape": self.tensors.shape, "mask_shape": self.mask.shape}
