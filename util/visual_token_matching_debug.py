...
class VTMMatchingModule(nn.Module):
    def __init__(self, dim_w, dim_z, n_attn_heads, n_levels):
        super().__init__()
        self.matching = nn.ModuleList([CrossAttention(dim_w, dim_z, dim_z, num_heads=n_attn_heads)
    ...
    def forward(self, W_Qs, W_Ss, Z_Ss, attn_mask=None):
        ...
        for level in range(self.n_levels):
            ...
            O, A = self.matching[level](Q, K, V, N=N, H=H, mask=attn_mask, get_attn_map=True)
