...
def init_distributed_mode(args):
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        print("Using os.environ RANK and WORLD_SIZE")
        ...
        print(f"Hello from rank {args.rank} of {args.world_size} on {gethostname()} where there are" \
            f" {gpus_per_node} allocated GPUs per node, and GPU that python found: {torch.cuda.device_count()}", flush=True)

    elif 'rank' in args.keys() and "SLURM_PROCID" in os.environ and not os.environ["SLURM_JOB_PARTITION"] == "interactive":
        # SLURM job launched with srun
        # Please note that you have to assign world size and rank yourself.
        print("Using slurm mode")

        # we only support multinode with only 1 gpu per node!
        gpus_per_node = 1 #int(os.environ["SLURM_GPUS_ON_NODE"])
        print(f"Hello from rank {args.rank} of {args.world_size} on {gethostname()} where there are" \
            f" {gpus_per_node} allocated GPUs per node, and GPU that python found: {torch.cuda.device_count()}", flush=True)
        assert gpus_per_node == int(os.environ["SLURM_GPUS_ON_NODE"]), "we now only support multinode with only 1 gpu per node!"
        assert gpus_per_node == torch.cuda.device_count(), "gpus_per_node: {}, torch.cuda.device_count():{}".format(gpus_per_node, torch.cuda.device_count()) 
        print(f"Hello from rank {args.rank} of {args.world_size} on {gethostname()} where there are" \
            f" {gpus_per_node} allocated GPUs per node.", flush=True)
        args.gpu = args.rank - gpus_per_node * (args.rank // gpus_per_node)
