...
def adjust_learning_rate(
    ...
    if args.schedule == "step":
        ...
        if curr_step < num_warmup_steps:
            ...
            if args.lr_min != 0.:
                # linearly decrease to lr_min
                text_encoder_gamma = max(
                    args.lr_min / args.lr,
                    (args.lr_min + (args.lr - args.lr_min) * float(num_training_steps - curr_step)
                    / float(max(1, num_training_steps - num_warmup_steps))) / args.lr,
                )
    ...
    elif args.schedule == "multistep_linear_with_warmup":
        if epoch > int(args.epochs - args.lr_refine_epochs):
            # double lr drop for the last few steps
            gamma = 0.1 ** int((epoch // args.lr_drop)+1)
        else:
            # normal lr drop
            gamma = 0.1 ** (epoch // args.lr_drop)
        if curr_step < num_warmup_steps:
    ...
    if "train_flags" in args and "vos" in args.train_flags and args.train_flags.vos.train_mask_head_in_ft_mode:
        base_lrs = [args.lr, args.lr_backbone, args.text_encoder_lr, args.lr * 10]
        gammas = [gamma, gamma, text_encoder_gamma, gamma]
